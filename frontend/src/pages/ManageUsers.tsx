...
interface UserProfile {
  ...
  teacher_type?: string;
  instructor_type?: string; // backwards compat alias
  stream?: string;
  student_id?: string;
  bio?: string;
  phone?: string;
...
interface Stats {
  total: number;
  students: number;
  teachers: number; // was: instructors
  admins: number;
  parents: number;
  non_teaching: number;
  western: number;
...
const BLANK_FORM: FormData = {
  ...
  email: "",
  role: "student",
  department: "western", // overridden in openCreate() with callerDept
  student_class: "",
  class_section: "",
  teacher_type: "",
  bio: "",
...
// Roles that can create/assign admin-level accounts
...
const ManageUsers: React.FC = () => {
  // Caller's role — used to conditionally hide/show UI elements
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      const params: Record<string, string> = {
        page_size: "200", // ← FIX: was missing, caused only 10 users to show
      };
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-3 sm:p-6">
      <div className="max-w-7xl mx-auto space-y-5">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          {/* school_admin: department filter hidden since they only see their dept */}
          {!isSchoolAdmin && (
            <select
              value={filterDept}
                      ...
                      {form.role === "student" && (
                        <div>
                          ...
                          <p className="text-[10px] text-gray-400 mt-1">
                            Sections are organisational only. All students in
                            the same base class share the same subjects.
                          </p>
                        </div>
                      )}
                    </div>
