...
const heroBg = "linear-gradient(135deg,#2e1065 0%,#4c1d95 50%,#3b0764 100%)";
const dotPat = {
  backgroundImage:
...
const ProgrammingDashboard: React.FC = () => {
  ...
  const loadPubResults = useCallback(async () => {
    if (!pubClass) {
      setPubError("Select a track first");
      return;
    }
    setPubLoading(true);
    setPubLoaded(false);
  ...
  const TABS: { id: Tab; label: string }[] = [
    ...
    { id: "teachers", label: `👨‍🏫 Instructors (${teachers.length})` },
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-10">
          {["const", "let", "=>", "{}", "[]", "async", "await"].map((kw, i) => (
            <span
              key={i}
              className="absolute text-violet-300 font-mono text-xs"
              style={{ left: `${10 + i * 13}%`, top: `${20 + i * 8}%` }}
            >
              {kw}
            </span>
          ))}
        ...
        <div className="relative max-w-6xl mx-auto flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-violet-200 text-sm">
                {students.length} students · {teachers.length} instructors ·{" "}
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-2 rounded-xl text-sm font-semibold"
              style={{
                background: A.violet + "30",
                color: A.violet,
                border: `1px solid ${A.violet}50`,
              }}
            >
              📚 Courses
            </button>
        ...
        <div className="max-w-6xl mx-auto px-6 flex overflow-x-auto">
          {TABS.map((t) => (
            <button
              ...
              style={
                activeTab === t.id
                  ? { borderColor: A.violet, color: A.violet }
                  : {}
              }
            >
              {t.label}
          ...
          >
              ...
              <div className="grid lg:grid-cols-3 gap-6">
                      ...
                      >
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-slate-400">
                            {c.student_class?.replace(/_/g, " ")}
                          </p>
                        </div>
                        <span
                          className="text-[10px] px-2 py-0.5 rounded-full font-semibold shrink-0"
                          style={{
                            background: A.violet + "15",
                            color: A.violet,
                          }}
                        >
                          Active
                ...
                <div className="space-y-4">
                  ...
                  <div
                    ...
                    style={{
                      background: A.violet + "08",
                      border: `1px solid ${A.violet}25`,
                    }}
                  >
                    <p>
                      <span className="text-slate-400">$ </span>
                      <span style={{ color: A.violet }}>echo</span>
            ...
            {(activeTab === "students" || activeTab === "teachers") &&
              (() => {
                ...
                const label =
                  activeTab === "teachers" ? "instructors" : "students";
                return (
                  <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                    <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800">
                      <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">
                        {list.length} {label} in Digital Technology department
                      </p>
                    </div>
                    <table className="w-full text-sm">
                      <thead className="bg-slate-800 text-white text-xs">
                        <tr>
                          ...
                          <th className="px-5 py-3 text-left">Track / Type</th>
                          ...
                          >
                            ...
                            <td className="px-5 py-3">
                              <div className="flex items-center gap-3">
                                <div
                                  ...
                                  style={{
                                    background: `linear-gradient(135deg,${A.violet},${A.cyan})`,
                                  }}
                                >
                                  {(
                                    p.user?.first_name?.[0] ??
                            ...
                            <td className="px-5 py-3">
                              ...
                              >
                                {p.student_class
                                  ? p.class_section
                                    ? `${p.student_class.replace(/_/g, " ")} (${p.class_section})`
                                    : p.student_class.replace(/_/g, " ")
                                  : p.teacher_type || "—"}
                              </span>
                            </td>
            ...
            {activeTab === "publish" && (
              <div className="space-y-5">
                <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                  <div className="px-5 py-4 bg-gradient-to-r from-violet-700 to-violet-600">
                    <h3 className="font-black text-white">
                      📝 Publish Results
                    </h3>
                    <p className="text-violet-100 text-xs mt-0.5">
                      Select track + term + session → publish to students
                    </p>
                  </div>
                  <div className="p-5 grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-3 py-2.5 border border-slate-300 dark:border-slate-600 rounded-xl text-sm dark:bg-slate-800 dark:text-white focus:ring-2 focus:ring-violet-500"
                      >
                        <option value="">— Select track —</option>
                        {PROG_CLASSES.map(([v, l]) => (
                    ...
                    <div className="flex items-end">
                      <button
                        ...
                        className="w-full px-4 py-2.5 bg-violet-700 hover:bg-violet-800 disabled:bg-slate-300 dark:disabled:bg-slate-700 text-white rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-colors"
                      >
                        {pubLoading ? (
                          <>
                        ...
                        {can > 0 ? (
                          ...
                          <div className="bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-5 text-center">
                            ...
                            <p className="text-sm text-slate-400 mt-1">
                              Instructors must enter and submit first
                            </p>
                          </div>
                        )}
                        {pubResults.length > 0 && (
                ...
                {!pubLoaded && !pubLoading && (
                  <div className="bg-white dark:bg-slate-900 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 p-16 text-center">
                    ...
                    <p className="text-sm text-slate-400">
                      Select track, term and session then click Load Results
                    </p>
                  </div>
                )}
              </div>
